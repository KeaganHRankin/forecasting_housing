...
starts_csv <- readr::read_csv("C:/Users/Keagan Rankin/OneDrive - University of Toronto/Saxe - Rankin/Project 2. Housing Projections/Data/houses/can_historic_starts.csv")
...
startsy_cv_models |> forecast(h=forecast_years) |> accuracy(starts_year)

# Which forecast looks best for h step forward forecast.
fc <- startsy_cv_models |>
  forecast(h = forecast_years) |>
  group_by(.id) |>
  mutate(h = row_number()) |>
  ungroup() |>
...
#--
# BASELINE EVALS:
# The mean model has lower RMSE at 8 steps but residuals are autocorrolated.
# it also fails the ljung box test
# The naive model is uncorrelated, okay baselines but can be improved
#--
...
starts_train_ets |> forecast(h=forecast_years) |> accuracy(starts_year) |> arrange(RMSE)

# best fitting are MNA, AadN, and ANN
#Refit to data
starts_fit_ets <- starts_year |> 
  model(ETS(`Total units` ~ error("M") + trend("N")))

starts_fit_ets_aad <- starts_year |> 
  model(ETS(`Total units` ~ error("A") + trend("Ad")))

starts_fit_ets_ann <- starts_year |> 
  model(ETS(`Total units` ~ error("A") + trend("N")))

# Model MN performs better RMSE, all are not autocorrolated,
# all models pass the ljung box test. Likely want to average
starts_fit_ets |> gg_tsresiduals() # residuals not normal, bootstrap.
augment(starts_fit_ets) |> features(.innov, ljung_box, lag=8)

starts_fit_ets_aad |> gg_tsresiduals() # residuals not normal, bootstrap.
augment(starts_fit_ets_aad) |> features(.innov, ljung_box, lag=8)

starts_fit_ets_ann |> gg_tsresiduals() # normal.
augment(starts_fit_ets_ann) |> features(.innov, ljung_box, lag=8)

# Forecast final model
starts_fc_ets <- starts_fit_ets |> forecast(h=forecast_years, bootstrap=TRUE)
starts_fc_ets_aad <- starts_fit_ets_aad |> forecast(h=forecast_years, bootstrap=TRUE)
starts_fc_ets_ann <- starts_fit_ets_ann |> forecast(h=forecast_years)
...
ets_plot <-starts_fc_ets |>
  ...
  #autolayer(starts_fc_ets_ann, level=conf_int, 
  #          size=1.5, color='light blue', alpha=0.7) +
  ylim(75000,400000) +
  theme_classic(base_size=20) +
  labs(title="Holt's ETS Model \n (MN and AAd)", y='Housing starts', x='Year') +
  theme(plot.title = element_text(face="bold", size=22),
        legend.position='none')
...
starts_train_arima |> forecast(h=forecast_years) |> accuracy(starts_year) |> arrange(RMSE)
glance(starts_train_arima) |> arrange(AICc)


# 111, 110, 210 all perform about the same AIC, but
# model ar111 has the best RMSE in the test so we choose this one.

starts_train_arima |> select(ar110) |> gg_tsresiduals()
...
starts_fit_arima <- starts_year |> 
  model(ARIMA(`Total units`~pdq(1,1,1))) #,stepwise=FALSE, approx=FALSE))
...
arima_plot <-starts_fc_arima |>
  ...
  ylim(75000,400000) +
  theme_classic(base_size=20) +
  labs(title="111 ARIMA Model", y='Housing starts', x='Year') +
  theme(plot.title = element_text(face="bold", size=22),
        legend.position='none')
...
# passes all of these tests well

starts_train_NN |> forecast(h=forecast_years, times=1000) |> accuracy(starts_year)
# it doesn't suck like Ontario but RMSE is higher than the other models
...
# Forecast final model
starts_fc_NN <- starts_fit_NN |> forecast(h=forecast_years, times=1000)
...
starts_year |> 
  ...
  forecast(h=forecast_years) |> autoplot(starts_year) +
...
bagged_plot <- autoplot(bagged, size=1.5, color='blue') +
  ...
  ylim(75000,400000) +
  theme_classic(base_size=20) +
  labs(title="Decomposed Bagged \nModels (ETS, ARIMA)", y='Housing starts', x='Year') +
  theme(plot.title = element_text(face="bold", size=22))
...
fc_list <- list(starts_fc_baseline |> filter(.model == 'Mean'),
...
ulm.data <- data.frame(
  ...
  Mean = ulm_list[2:4] |> sapply(sum),
  Naive = ulm_list[7:9] |> sapply(sum),
  `Arima_111` = ulm_list[12:14] |> sapply(sum),
  `ETS` = ulm_list[17:19] |> sapply(sum),
...
label_scatter <- tibble(
  differential = c(5,3.8), y = c(2200000, 6000000), 
  label = c("CMHC BAU", 
            "CMHC 2030 estimate to achieve affordability"),
  color = c("#737373", "#8B1E3F")
)
...
ggplot(ulm.data|> melt(id.vars=c('Interval')), 
  ...
  geom_hline(yintercept = 2000000, size = 1, color = "#737373", linetype='dotted') +
  geom_hline(yintercept = 5800000, size = 1, color = "#8B1E3F", linetype='dotted') +
  theme_classic(base_size=20) +
